...
WHERE country_code LIKE 'X%' OR country_region = 'Aggregates'
""")
```
...
SELECT *
FROM world_bank_data.wdi_country
WHERE country_code NOT LIKE 'X%' AND country_region != 'Aggregates';
""")
```
...
run_sql_and_return_html(cnx,"""
SELECT 
    country_income_group AS "Income Category",
    COUNT(*) AS "Number of Countries"
FROM wdi_country
...
SELECT 
    country_name AS "Country Name",
    country_income_group AS "Income Group"
FROM wdi_country
WHERE country_income_group IS NULL OR country_income_group = '';
""")
```
...
ORDER BY country_region, COUNT(*) DESC;
""")
```
...
SELECT 
    ...
    SUM(CASE WHEN country_income_group IS NULL OR country_income_group = '' THEN 1 ELSE 0 END) AS "Other",
    COUNT(*) AS "Total"
FROM wdi_country
GROUP BY country_region
ORDER BY country_region;
...
run_sql_and_return_html(cnx,"""
SELECT 
    w1.country_code AS "Country Code",
    w1.country_name AS "Country Name",
    w1.country_region AS "Region",
    w1.country_income_group AS "Income Group"
FROM wdi_country w1
JOIN (
...
ON w1.country_region = w2.country_region 
AND w1.country_income_group = w2.country_income_group
ORDER BY w1.country_name;
""")
```
...
WITH all_combinations AS (
    SELECT DISTINCT
        r.country_region AS region,
        i.country_income_group AS income_group
    FROM 
        (SELECT DISTINCT country_region FROM wdi_country) r
    CROSS JOIN
        (SELECT DISTINCT country_income_group FROM wdi_country WHERE country_income_group IS NOT NULL AND country_income_group != '') i
),
existing_combinations AS (
    SELECT 
        country_region AS region,
        country_income_group AS income_group,
        COUNT(*) AS count
    FROM wdi_country
    WHERE country_income_group IS NOT NULL AND country_income_group != ''
    GROUP BY country_region, country_income_group
)
SELECT 
    a.region AS "Region",
...
LEFT JOIN existing_combinations e ON a.region = e.region AND a.income_group = e.income_group
WHERE e.region IS NULL
ORDER BY a.region, a.income_group;
""")
```
...
SELECT 
    region AS "Region",
    ROUND(SUM(CASE WHEN income_group = 'High income' THEN count * 100.0 / (SELECT total FROM overall_total) ELSE 0 END), 2) AS "High Income %",
    ROUND(SUM(CASE WHEN income_group = 'Upper middle income' THEN count * 100.0 / (SELECT total FROM overall_total) ELSE 0 END), 2) AS "Upper Middle %",
    ROUND(SUM(CASE WHEN income_group = 'Lower middle income' THEN count * 100.0 / (SELECT total FROM overall_total) ELSE 0 END), 2) AS "Lower Middle %",
    ROUND(SUM(CASE WHEN income_group = 'Low income' THEN count * 100.0 / (SELECT total FROM overall_total) ELSE 0 END), 2) AS "Low Income %",
    ROUND(SUM(CASE WHEN income_group IS NULL OR income_group = '' THEN count * 100.0 / (SELECT total FROM overall_total) ELSE 0 END), 2) AS "Other %",
