...
WITH all_combinations AS (
    ...
    CROSS JOIN (SELECT DISTINCT `Income Group` FROM wdi_country WHERE `Income Group` IS NOT NULL) i
),
...
SELECT a.`Region`, a.`Income Group`
FROM all_combinations a
LEFT JOIN existing_combinations e ON a.`Region` = e.`Region` AND a.`Income Group` = e.`Income Group`
...
SELECT 
    Region,
    ROUND(SUM(CASE WHEN `Income Group` = 'High income' THEN count * 100.0 / (SELECT total FROM overall_total) ELSE 0 END), 1) AS 'High Income %',
    ROUND(SUM(CASE WHEN `Income Group` = 'Upper middle income' THEN count * 100.0 / (SELECT total FROM overall_total) ELSE 0 END), 1) AS 'Upper Middle %',
    ROUND(SUM(CASE WHEN `Income Group` = 'Lower middle income' THEN count * 100.0 / (SELECT total FROM overall_total) ELSE 0 END), 1) AS 'Lower Middle %',
    ROUND(SUM(CASE WHEN `Income Group` = 'Low income' THEN count * 100.0 / (SELECT total FROM overall_total) ELSE 0 END), 1) AS 'Low Income %',
    ROUND(SUM(count) * 100.0 / (SELECT total FROM overall_total), 1) AS 'Total %'
FROM counts
GROUP BY Region
ORDER BY Region;
""")
...
SELECT 
    ...
    count AS "Number of Countries",
    (SELECT total FROM overall_total) AS "Total Countries",
    ROUND(count * 100.0 / (SELECT total FROM overall_total), 1) AS "Percentage"
FROM income_counts
ORDER BY count DESC;
""")
```
